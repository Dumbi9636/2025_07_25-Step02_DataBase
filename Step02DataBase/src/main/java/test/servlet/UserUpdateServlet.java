...
public class UserUpdateServlet extends HttpServlet {
	...
	// 이 서블릿(UserUpdateServlet)이 초기화되는 시점에 최초 1번 호출되는 메소드
	@Override
	public void init() throws ServletException {
		// 무언가 초기화 작업을 여기서 하면된다. 
		ServletContext context = getServletContext();
		// web.xml 파일에 "fileLocation" 이라는 이름을 저장된 정보를 읽어와서 필드에 저장하기
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// DB 에서 사용자 정보를 불러온다  밑에서 dao 많이 쓰니까 객체 만들어둠 
		UserDto dto=UserDao.getInstance().getByUserName(userName);
		
		// 만일 업로드된 프로필 이미지가 있다면(수정하지 않았으면 없다)
		if(filePart!=null && filePart.getSize()>0) {
			// 원본 파일의 이름 얻어내기, filePart 의 메소드를 이용하면 추출할 수 있다. 외울 필요없고 이해만 하면 됨. 
			 ...
			 * 해당 파일에서 byte 알갱이를 읽어들일수있는 InputStream 객체를 얻어내서 
			 */
			InputStream is=filePart.getInputStream();
			// 원하는 목적지에 copy 를 해야한다 files에 copy 메소드가 있음. 거기 안에 inputstream 객체의 정보를 저장
