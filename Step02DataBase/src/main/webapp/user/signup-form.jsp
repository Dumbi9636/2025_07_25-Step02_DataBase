...
<body>
	...
	<div class="container">
		...
		<form action="signup.jsp" method="post" novalidate>
			...
			<button class="btn btn-warning btn-sm fw-bold" type="submit" disabled>가입하기</button>
		</form>
	</div>
	<script>
		/*
			아이디, 비밀번호, 유효성 요소 중 하나라도 false 인 경우
			가입하기 버튼에 disabled 리턴하기 위한 변수 및 함수 생성 
		*/ 
		...
		const checkForm = () =>{
			...
			if(isIdValid && isPwdValid && isEmailValid){
				// 전송 버튼에 disabled 속성을 제거하고 (removeAttribute <-> setAttribute)
				document.querySelector("[type=submit]").removeAttribute("disabled");
			}
		}
	
		// blur 는 포커스를 잃었을때 발생하는 이벤트(input은 입력 당시)
		...
		/*
			- 정규표현식에서 줄인 표현                 <줄인표현>
			특수문자가 아닌 한글자 => [a-zA-Z0-9_]  => [\w] 소문자
			특수문자 한글자 => [^a-zA-Z0-9_]      => [\W] 대문자
			숫자 한글자 =>[0-9]                  =>[\d] 
			숫자가 아닌 한글자 => [0-9]            =>[\D] 
			공백 한글자 => [ ]                   => [\s] 소문자
			공백이 아닌 한글자 => [^ ]             => [\S] 대문자
		*/
		...
		const checkPwd = ()=>{
			...
			if( !reg_pwd.test(pwd) || !reg_pwd.test(pwd2) ){
				...
				// 사용할 수 없는 비밀번호라는 의미에서 false를 넣어준다
				isPwdValid=false;
				checkFrom();
				return;
			}
		...
		document.querySelector("#email").addEventListener("input", (e)=>{
			...
			if(reg_email.test(email)){
				...
				// 이메일이 유효하다는 의미에서 true를 넣어준다
				isEmailValid=true;
			}else{
				document.querySelector("#email").classList.add("is-invalid");
				// 이메일이 유효하지 않다는 의미에서 false를 넣어준다
				isEmailValid=false;
			}
			checkFrom();
		});	
